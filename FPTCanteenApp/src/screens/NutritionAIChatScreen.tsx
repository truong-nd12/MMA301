...
export default function NutritionAIChatScreen({ navigation }: any) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: "Xin chào! Tôi là trợ lý AI thông minh. Tôi có thể giúp bạn:\n\n• Trả lời bất kỳ câu hỏi nào\n• Tư vấn dinh dưỡng và sức khỏe\n• Hỗ trợ học tập và công việc\n• Giải trí và trò chuyện\n• Và nhiều điều khác!\n\nBạn muốn hỏi gì?",
      isUser: false,
      timestamp: new Date(),
      type: "text",
    },
  ...
  const getAiTypeInfo = () => {
    ...
    } else if (aiService.isHuggingFaceEnabled()) {
      return {
        type: "🌐 Hugging Face",
        color: "#28a745",
        description: "AI miễn phí",
  ...
  useEffect(() => {
    // Tự động load API key từ storage và cấu hình
    initializeAI();
  }, []);

  // Auto-enable Gemini on mount
  useEffect(() => {
    if (aiService.isGeminiEnabled()) {
  ...
  const initializeAI = async () => {
    try {
      ...
      if (aiService.isGeminiEnabled()) {
        ...
        // Test Gemini AI
        const testResponse = await aiService.getResponse(
          "test",
          userNutritionData
        );
        console.log("✅ Gemini AI working perfectly");
      } else {
        // Fallback to Local AI
        aiService.setUseLocalAI(true);
        setIsAIConnected(false);
        console.log("🔄 Using AI Local - Free and unlimited!");
        ...
        console.log("✅ AI Local working perfectly");
      }
    } catch (error) {
