  ...
  if (currentStatus === "cancelled") {
    return (
      <View style={styles.timelineContainer}>
        <View style={[styles.cancelledTimeline, { backgroundColor: '#F44336' }]}>
    ...
    <View style={styles.timelineContainer}>
      {steps.map((step, index) => (
        <TimelineStep
          key={step}
          status={step}
          isActive={currentStep === (statusConfig[step]?.step || 0)}
          isCompleted={currentStep > (statusConfig[step]?.step || 0)}
...
export default function OrderTrackingScreen({ navigation }: any) {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      if (response.success && response.orders) {
        // Add estimatedTime to each order if missing
        const ordersWithEstimatedTime = response.orders.map(order => ({
          ...order,
          estimatedTime: (order as any).estimatedTime || "Đang cập nhật"
        }));
        setOrders(ordersWithEstimatedTime);
  ...
  const renderOrderCard = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      ...
      >
        {/* Status Header */}
        <View style={[styles.orderStatusHeader, { backgroundColor: config.color }]}>
        ...
        <View style={styles.orderContent}>
          ...
          <View style={styles.cardActions}>
            ...
            >
              <View style={[styles.detailButtonGradient, { backgroundColor: '#667eea' }]}>
  ...
  const renderEmptyState = () => (
    <Animatable.View animation="fadeInUp" style={styles.emptyState}>
      <View style={[styles.emptyIconContainer, { backgroundColor: '#FF6B6B20' }]}>
      ...
      {!searchQuery && selectedFilter === "all" && (
        ...
        >
          <View style={[styles.orderNowGradient, { backgroundColor: '#FF6B6B' }]}>
    ...
    return (
      <View style={styles.loadingContainer}>
        <Animatable.View animation="pulse" iterationCount="infinite">
          <View style={[styles.loadingIconContainer, { backgroundColor: '#FF6B6B20' }]}>
            <Ionicons name="receipt-outline" size={64} color="#FF6B6B" />
          </View>
        </Animatable.View>
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        {renderFilterButton("processing", "Chuẩn bị", "restaurant-outline")}
        {renderFilterButton("shipped", "Sẵn sàng", "checkmark-circle-outline")}
